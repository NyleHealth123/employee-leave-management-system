...
import static com.example.leavemanagement.request.domain.LeaveRuleViolation.Reason.NO_CHARGEABLE_DAYS;
import static com.example.leavemanagement.request.domain.LeaveRuleViolation.Reason.VALIDATION_FAILED;
...
public class LeaveDurationCalculator {
    public LeaveCalculation calculate(LocalDate start,LocalDate end,DurationMode mode,boolean allowsHalfDay,boolean excludeWeeklyOffs,Set<Integer> weeklyOffDays,boolean excludeHolidays,Set<LocalDate> holidays){
        if(start==null||end==null||mode==null)throw new LeaveRuleViolation(VALIDATION_FAILED,"Dates and duration mode are required");
        if(start.isAfter(end))throw new LeaveRuleViolation(VALIDATION_FAILED,"Start date must not be after end date");
        if(mode!=DurationMode.FULL_DAY){if(!allowsHalfDay)throw new LeaveRuleViolation(VALIDATION_FAILED,"This leave type does not allow half days");if(!start.equals(end))throw new LeaveRuleViolation(VALIDATION_FAILED,"A half day must use one date");}
        ...
        if(slots.isEmpty())throw new LeaveRuleViolation(NO_CHARGEABLE_DAYS,"Configured rules exclude every requested date");

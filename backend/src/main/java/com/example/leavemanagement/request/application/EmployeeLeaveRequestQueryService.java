...
public class EmployeeLeaveRequestQueryService {
    private final LeaveRequestRepository requests;private final LeaveRequestStatusHistoryRepository history;private final LeavePolicyQueryService policies;private final EmployeeProfileRepository employees; private final Clock clock;
    public EmployeeLeaveRequestQueryService(LeaveRequestRepository requests,LeaveRequestStatusHistoryRepository history,LeavePolicyQueryService policies,EmployeeProfileRepository employees,Clock clock){this.requests=requests;this.history=history;this.policies=policies;this.employees=employees;this.clock=clock;}
    ...
    public Detail detail(LeaveRequestEntity request){var employee=employees.findById(request.getEmployeeId()).orElseThrow();var entries=history.findAllByRequestIdOrderByCreatedAtAsc(request.getId()).stream().map(h->new HistoryEntry(h.getFromStatus(),h.getToStatus(),employee.getDisplayName(),h.getComment(),h.getCreatedAt())).toList();var s=summary(request);var policy=policies.effective(request.getLeaveTypeId(),request.getStartDate());var eligibility=com.example.leavemanagement.request.domain.CancellationPolicy.evaluate(request.getEmployeeId().equals(employee.getId()),request.getStatus(),request.getStartDate(),policy.getCancellationCutoffDays(),Instant.now(clock),clock.getZone());return new Detail(s.id(),s.employeeId(),s.employeeName(),s.leaveTypeId(),s.leaveTypeName(),s.startDate(),s.endDate(),s.durationMode(),s.chargeableDays(),s.status(),s.submittedAt(),s.version(),request.getReason(),request.getDecisionComment(),eligibility.allowed(),eligibility.blockedReason(),entries);}

...
class LeaveDurationCalculatorTest {
    ...
    @Test void rejectsInvertedRange(){assertThatThrownBy(()->calculator.calculate(LocalDate.of(2026,8,20),LocalDate.of(2026,8,19),DurationMode.FULL_DAY,true,false,Set.of(),false,Set.of())).isInstanceOf(LeaveRuleViolation.class).extracting("reason").isEqualTo(LeaveRuleViolation.Reason.VALIDATION_FAILED);}
    @Test void rejectsDisallowedHalfDay(){assertThatThrownBy(()->calculator.calculate(LocalDate.of(2026,8,19),LocalDate.of(2026,8,19),DurationMode.HALF_DAY_AM,false,false,Set.of(),false,Set.of())).isInstanceOf(LeaveRuleViolation.class);}
    @Test void rejectsHalfDayDateRange(){assertThatThrownBy(()->calculator.calculate(LocalDate.of(2026,8,19),LocalDate.of(2026,8,20),DurationMode.HALF_DAY_PM,true,false,Set.of(),false,Set.of())).isInstanceOf(LeaveRuleViolation.class).extracting("reason").isEqualTo(LeaveRuleViolation.Reason.VALIDATION_FAILED);}
    @Test void rejectsZeroChargeableDays(){assertThatThrownBy(()->calculator.calculate(LocalDate.of(2026,8,22),LocalDate.of(2026,8,23),DurationMode.FULL_DAY,true,true,Set.of(6,7),false,Set.of())).isInstanceOf(LeaveRuleViolation.class).extracting("reason").isEqualTo(LeaveRuleViolation.Reason.NO_CHARGEABLE_DAYS);}
    ...
    @Test void rejectsNullOrEmptyInput(){assertThatThrownBy(()->calculator.calculate(null,LocalDate.of(2026,8,19),DurationMode.FULL_DAY,true,false,Set.of(),false,Set.of())).isInstanceOf(LeaveRuleViolation.class).extracting("reason").isEqualTo(LeaveRuleViolation.Reason.VALIDATION_FAILED);}

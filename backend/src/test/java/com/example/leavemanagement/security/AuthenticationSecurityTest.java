...
class AuthenticationSecurityTest {
 ...
 @Test void csrfBootstrapIsAvailableBeforeLogin()throws Exception{mvc.perform(get("/api/auth/csrf")).andExpect(status().isOk()).andExpect(cookie().httpOnly("XSRF-TOKEN",true)).andExpect(jsonPath("$.headerName").value("X-XSRF-TOKEN")).andExpect(jsonPath("$.token").isNotEmpty());}
 @Test @DirtiesContext(methodMode=DirtiesContext.MethodMode.BEFORE_METHOD) void csrfBootstrapTokenAuthenticatesValidLogin()throws Exception{var id=UUID.randomUUID();when(manager.authenticate(any())).thenReturn(UsernamePasswordAuthenticationToken.authenticated(id.toString(),"n/a",List.of(new SimpleGrantedAuthority("ROLE_EMPLOYEE"))));when(actors.require()).thenReturn(new CurrentActor(id,UUID.randomUUID(),null,Set.of("EMPLOYEE"),"Employee"));var csrf=csrfBootstrap();mvc.perform(post("/api/auth/login").cookie(csrf.cookie()).header(csrf.headerName(),csrf.token()).contentType(MediaType.APPLICATION_JSON).content("{\"login\":\"employee\",\"password\":\"secret\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.roles[0]").value("EMPLOYEE"));verify(manager).authenticate(any());}
 ...
 @Test @DirtiesContext(methodMode=DirtiesContext.MethodMode.BEFORE_METHOD) void invalidCredentialsWithBootstrapCsrfAreGeneric()throws Exception{when(manager.authenticate(any())).thenThrow(new BadCredentialsException("bad"));var csrf=csrfBootstrap();mvc.perform(post("/api/auth/login").cookie(csrf.cookie()).header(csrf.headerName(),csrf.token()).contentType(MediaType.APPLICATION_JSON).content("{\"login\":\"unknown\",\"password\":\"wrong\"}")).andExpect(status().isUnauthorized()).andExpect(jsonPath("$.code").value("INVALID_CREDENTIALS")).andExpect(jsonPath("$.detail").value("Invalid login or password"));}
 ...
 private CsrfBootstrap csrfBootstrap()throws Exception{MvcResult result=mvc.perform(get("/api/auth/csrf")).andExpect(status().isOk()).andReturn();var body=result.getResponse().getContentAsString();return new CsrfBootstrap(JsonPath.read(body,"$.token"),JsonPath.read(body,"$.headerName"),result.getResponse().getCookie("XSRF-TOKEN"));}

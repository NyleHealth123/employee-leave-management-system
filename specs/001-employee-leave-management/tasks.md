...
- [X] T106 [P] [US5] Write PostgreSQL reporting tests for inclusive period boundaries, status/type aggregation, decimal-day conversion, stable sorting, pagination bounds, and relevant indexes in `backend/src/test/java/com/example/leavemanagement/integration/LeaveReportingRepositoryTest.java` (FR-029, FR-030)
...
- [X] T109 [P] [US5] Write frontend tests for administrator report period validation, status/type summary states, organization request pagination, audit filtering, safe errors, keyboard access, and responsive layouts in `frontend/src/features/reports/AdminReports.test.tsx`

### Implementation for User Story 5

- [X] T110 [P] [US5] Implement paginated organization request projections with date/status filters and stable sorting in `backend/src/main/java/com/example/leavemanagement/reporting/persistence/OrganizationLeaveRequestRepository.java` (FR-029)
- [X] T111 [P] [US5] Implement status/type aggregate report queries with inclusive selected-period semantics and integer-unit-to-day conversion in `backend/src/main/java/com/example/leavemanagement/reporting/persistence/LeaveSummaryRepository.java` (FR-030)
- [X] T112 [P] [US5] Implement immutable audit event filtering/pagination projections in `backend/src/main/java/com/example/leavemanagement/audit/application/AuditQueryService.java` (FR-027, FR-028)
...
- [X] T117 [US5] Run US5 suites and compare report totals with source request units across all four statuses and multiple leave types in `backend/src/test/java/com/example/leavemanagement/integration/LeaveReportingRepositoryTest.java`
...
- [X] T118 [P] Add the strictly profile-isolated, repeatable local-demo Flyway seed at `backend/src/main/resources/db/local-demo/R__local_demo_data.sql`. Seed at least 50 and preferably approximately 50–60 synthetic employee records with no real personal information; at least one administrator account; multiple manager accounts; employee accounts; realistic employee-to-manager relationships with direct reports; and only existing approved model fields for roles, leave types, policies, applicable weekly offs, holidays, balances, and valid representative `PENDING`, `APPROVED`, `REJECTED`, and `CANCELLED` requests plus reporting/pagination, audit/history, manager-scope, cancellation/correction, and administrator-view coverage. Use deterministic/repeatable identifiers and data so recreation does not duplicate or corrupt rows. Supply demo credentials/secrets only from local environment configuration, preserve Spring Security password hashing, fail if required credential configuration is missing, never hardcode plaintext passwords in SQL/source/Git or fallback credentials, and ensure production profiles never activate `classpath:db/local-demo`, production startup does not depend on demo data, and demo loading fails closed outside local-demo. Verify employee count >=50, administrator exists, multiple managers/direct reports exist, representative statuses/configuration exist, deterministic recreation works, and production configuration cannot load the seed (FR-036, SC-011).
- [X] T119 [P] Add OpenAPI drift and CSRF contract validation to Maven verification against `specs/001-employee-leave-management/contracts/openapi.yaml`, including local reference resolution, required input-only `initialPassword`, shared closed/non-empty/unique role schemas, absence of credential response fields, login missing/malformed/validation-invalid body coverage as `400 VALIDATION_FAILED`, operation-specific applicable `400`/`401`/`403`/`404`/`409` responses, and a matrix proving every operation requiring `X-XSRF-TOKEN` documents and returns `403 ACCESS_DENIED` for missing/invalid tokens without protected state mutation, in `backend/pom.xml`, `backend/src/test/java/com/example/leavemanagement/contract/OpenApiValidationTest.java`, and `backend/src/test/java/com/example/leavemanagement/security/CsrfContractTest.java` (FR-001, FR-002, FR-032; SC-003)
- [X] T120 [P] Add full backend authorization coverage for every operation in `contracts/authorization.md`, including employee ownership, the employee same-manager/active coworker calendar predicate and privacy projection, manager direct-report scope, unconditional self-approval prevention, administrator-only operations, multi-role endpoint semantics, and forbidden no-mutation assertions in `backend/src/test/java/com/example/leavemanagement/security/AuthorizationMatrixTest.java` (FR-002, FR-003, FR-015, FR-016, FR-031, FR-035; SC-003, SC-009, SC-010)
...
- [X] T122 [P] Add responsive and keyboard-accessibility tests for role navigation, forms, dialogs, tables/cards, status text, focus management, and agenda views in `frontend/src/test/AccessibilityResponsive.test.tsx`
- [X] T123 Implement query timing assertions or explain-plan checks for indexed interactive, calendar, and report queries in `backend/src/test/java/com/example/leavemanagement/integration/QueryPerformanceTest.java` using the plan's 500 ms/2 s targets at representative MVP fixture size
- [X] T124 Implement focused Playwright smoke tests using the profile-isolated T118/T125 local-demo dataset for employee login/preview/submit, manager approve/reject within direct-report scope, employee balance/status refresh, eligible cancellation restoration, and administrator audit visibility in `e2e/tests/leave-management-smoke.spec.ts`
- [X] T125 [P] Add safe local-demo reset/cleanup support for the complete FR-036 dataset in `e2e/global-setup.ts` and `backend/src/main/java/com/example/leavemanagement/shared/config/LocalDemoConfiguration.java`; recreate at least 50 synthetic employees with deterministic valid manager/direct-report relationships, roles, leave configuration, balances, and representative `PENDING`/`APPROVED`/`REJECTED`/`CANCELLED` requests without unexpected duplicate rows; make the reset suitable for Playwright/E2E; refuse cleanup/reset against production profiles/configuration/databases and fail closed if environment/profile safety cannot be established.
- [X] T126 Run `backend/mvnw.cmd verify`, `frontend` lint/typecheck/test/build, and `e2e` smoke tests using the profile-isolated T118/T125 local-demo dataset where demo/E2E verification applies; record commands and any platform-specific notes in `README.md`; add automated six-layer architecture-boundary verification such as `backend/src/test/java/com/example/leavemanagement/architecture/ArchitectureLayerBoundaryTest.java`, using an existing dependency if available or adding the smallest appropriate test-only dependency to `backend/pom.xml` when T126 is implemented, and require it to verify that (1) API/Presentation does not directly depend on Persistence repositories/entities, (2) Domain does not depend on API/Presentation, (3) Persistence does not depend on API transport/controller concerns, (4) no prohibited circular dependencies exist between the six architectural layers, (5) Infrastructure/Security dependency direction does not bypass approved application/domain boundaries, (6) the existing modular-monolith module/package organization remains valid, and (7) the architecture test runs as part of backend Maven verification
- [X] T127 Execute every acceptance scenario in `specs/001-employee-leave-management/quickstart.md` against a clean local PostgreSQL volume initialized with the profile-isolated T118 local-demo dataset and reset/recreation support from T125; document verified results in `specs/001-employee-leave-management/quickstart.md`
...
- [X] T129 Review dependency versions, session/cookie/CSRF settings, secret handling, safe logging, and production profile defaults in `backend/pom.xml`, `backend/src/main/resources/application.yml`, `frontend/package-lock.json`, and `.env.example`
...
- [ ] T131 Re-run the traceability audit and reconcile implemented task/test identifiers for FR-001–FR-036 and SC-001–SC-011 in `specs/001-employee-leave-management/contracts/traceability.md`; include FR-036/SC-011 evidence for >=50 synthetic employees, an administrator account, multiple managers with direct reports, representative leave configuration/statuses, deterministic recreation, production isolation, credential secrecy, and E2E/reporting usability.

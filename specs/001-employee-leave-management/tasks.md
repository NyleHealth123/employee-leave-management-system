...
- [ ] T119 [P] Add OpenAPI drift validation to Maven verification against `specs/001-employee-leave-management/contracts/openapi.yaml`, including reference resolution, required input-only `initialPassword`, shared closed/non-empty/unique role schemas, absence of credential response fields, and operation-specific applicable `400`/`401`/`403`/`404`/`409` responses, in `backend/pom.xml` and `backend/src/test/java/com/example/leavemanagement/contract/OpenApiValidationTest.java` (FR-001, FR-002, FR-032)
